...
int main() {
	...
	InterestRate p(0.05); //continuous
	InterestRate q = InterestRate(0.04, 4); //quarterly
	InterestRate* r = new InterestRate(0.07, 12); //monthly
	...
	// or
	std::cout << "7% Monthly DF = " << r->discountFactor(5) << std::endl;
	std::cout << "7% Monthly Fwd = " << r->forwardRate(3,4) << std::endl;
	delete r;

	//NEW Classes
	CCRate p1(0.05);  //continuous
	DCRate q1 = DCRate(0.04, 4); //quarterly
	DCRate* r1 = new DCRate(0.07, 12);  //monthly
	DCRate* r2 = new DCRate(0.07, 12);  //monthly
